...
import { messages } from '@/data/conversations';
   ...
   function getConversationName(
     ...
     if (conv.type === 'general') return 'General Chat';
     if (conv.type === 'team') {
       const team = teams.find((t) => t.id === conv.teamId);
       return team ? 'Team ' + team.name : 'Team chat';
     }
     const otherUid = (conv.participantUids || []).find((uid) => uid !== currentUser.uid);
     if (!otherUid) return 'Private chat';
     const other = users.find((u) => u.uid === otherUid);
     return other ? other.displayName : 'Private chat';
   }
   ...
   function getConversationAvatar(
     ...
     if (conv.type === 'team') {
       const team = teams.find((t) => t.id === conv.teamId);
       return { text: team ? team.name.slice(0, 2).toUpperCase() : 'FT', variant: 'team' };
     }
     const otherUid = (conv.participantUids || []).find((uid) => uid !== currentUser.uid);
     const other = users.find((u) => u.uid === otherUid);
     ...
     if (conversations.length === 0) {
       return (
         <div className="empty" style={{ padding: 24 }}>
           <div className="empty__message">No conversations yet</div>
         </div>
       );
     }
