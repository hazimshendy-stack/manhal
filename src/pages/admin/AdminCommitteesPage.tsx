     ...
     const save = async () => {
       ...
       try {
         ...
         if (editing) {
           ...
           try { await logAudit(me, 'UPDATE_COMMITTEE', 'Committee', editing.id, form.nameAr); } catch {}
           toast.success('Updated');
         } else {
           const id = 'COMM-' + Date.now().toString(36).toUpperCase();
           await createOne('committees', { id, ...payload });
           try { await logAudit(me, 'CREATE_COMMITTEE', 'Committee', id, form.nameAr); } catch {}
           toast.success('Added');
         }
         close();
       } catch {
     ...
     const del = async () => {
       ...
       try {
         ...
         for (const m of members) {
           if (Array.isArray(m.committeeIds) && m.committeeIds.includes(toDelete.id)) {
             await updateOne('members', m.id, {
               committeeIds: m.committeeIds.filter((c) => c !== toDelete.id),
             });
           }
         }
         try { await logAudit(me, 'DELETE_COMMITTEE', 'Committee', toDelete.id, toDelete.nameAr); } catch {}
     ...
     return (
       <div className="admin-page">
         <PageHeader
           eyebrow="Admin"
           title="Committees"
           description="Add committees. Each member must join at least one."
         />

         <SectionHeader
           eyebrow="List"
           title={'Committees (' + committees.length + ')'}
           action={
         ...
         {loading ? (
           ...
           <EmptyState
             title="No committees"
             message="Add your first committee."
             action={
           ...
           <div className="grid grid--wide">
             {committees.map((c) => {
               const count = members.filter(
                 (m) => Array.isArray(m.committeeIds) && m.committeeIds.includes(c.id),
               ).length;
         ...
         <Modal
           ...
           title={editing ? 'Edit Committee' : 'New Committee'}
           onClose={close}
           footer={
             <>
         ...
         >
           <FormField label="Name (Arabic)" required>
             <TextInput value={form.nameAr} onChange={(v) => setForm({ ...form, nameAr: v })} />
           </FormField>
           ...
           <FormField label="Team" required>
             <Select
               value={form.teamId ?? 'helpers'}
               onChange={(v) => setForm({ ...form, teamId: v as TeamId })}
           ...
           <FormField label="Color">
             <Select
               ...
               options={[
                 { value: '#151A45', label: 'Navy' },
                 { value: '#C1272D', label: 'Red' },
         ...
         <ConfirmDialog
           ...
           message={'Delete "' + (toDelete?.nameAr || '') + '"?'}
           confirmLabel="Delete"
           danger
           busy={busy}
           onConfirm={del}
